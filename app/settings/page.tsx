...
function SettingsPage() {
  ...
  const loadMyBlog = async () => {
    try {
      const d = await api<typeof myBlogPosts>("/api/blog/mine");
      setMyBlogPosts(d);
    } catch { /* ignore */ }
  };
  ...
  const submitBlogPost = async () => {
    ...
    try {
      await api("/api/blog", {
        method: "POST",
        body: JSON.stringify({ title: blogTitle, body: blogBody, link_url: blogLink, status: "pending" }),
      });
  ...
  const loadMessages = async () => {
    try {
      const d = await api<typeof messages>("/api/messages/mine");
      setMessages(d);
    } catch { /* ignore */ }
  };
